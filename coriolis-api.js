...
/**
    This file was automatically generated by generate_api.py of https://github.com/Gobidev/coriolis-api
    
    Time of generation: 2026-04-02 14:51:17
    Build Hash: f83896c7c70125f311c1d54f99c029e628f214efd81adab824a498636afbbe59
**/
...
const CORE_INTERNAL_NAME_MAPPING = {
  fdToCoriolis: {
...
const CORIOLIS_TO_FD_BULKHEAD_NAME_MAPPING = BulkheadNames.reduce(
  (acc, cur, index) => {
    switch (cur) {
 ...
 */
function setQualityCB(blueprint, quality, cb) {
  // Pick given value as multiplier
...
class Ship {
  ...
  constructor(id, properties, slots) {
    ...
    for (let slotType in slots) {   // Initialize all slots
      ...
      for (let slot of slotGroup) {
        // utility mounts are a type of hardpoints and need to be different to be able to as SLEF
  ...
  calcSpeedsWith(fuel, cargo) {
    ...
    return speed(this.dryMass + fuel + cargo + reserveFuelMass, this.speed, this.standard[1].m, this.minthrust);
  }
  ...
  calcSpeed(eng, fuel, cargo, boost) {
    const reserveFuelMass = this.reserveFuelCapacity || 0;
    return calcSpeed(this.dryMass + fuel + cargo + reserveFuelMass, this.speed, this.standard[1].m, this.minthrust, eng, this.boost / this.speed, boost);
  }
  ...
  calcPitch(eng, fuel, cargo, boost) {
    const reserveFuelMass = this.reserveFuelCapacity || 0;
    return calcPitch(this.dryMass + fuel + cargo + reserveFuelMass, this.pitch, this.standard[1].m, this.pipSpeed, eng, this.boost / this.speed, boost);
  }
  ...
  calcRoll(eng, fuel, cargo, boost) {
    const reserveFuelMass = this.reserveFuelCapacity || 0;
    return calcRoll(this.dryMass + fuel + cargo + reserveFuelMass, this.roll, this.standard[1].m, this.pipSpeed, eng, this.boost / this.speed, boost);
  }
  ...
  calcYaw(eng, fuel, cargo, boost) {
    const reserveFuelMass = this.reserveFuelCapacity || 0;
    return calcYaw(this.dryMass + fuel + cargo + reserveFuelMass, this.yaw, this.standard[1].m, this.pipSpeed, eng, this.boost / this.speed, boost);
  }
  ...
  initializePreEngineeredModule(m) {
    ...
    for (let i = 0; i < m.preEngineered.blueprints.length; i++) {
      ...
      if (blueprint.grades[grade] && blueprint.grades[grade].features) {
        ...
        for (const featureName in features) {
          const modification = Dist.Modifications.modifications[featureName];
    ...
    if (m.preEngineered.experimentalEffects && m.preEngineered.experimentalEffects.length > 0) {
      ...
      if (Dist.Modifications.specials[specialName]) {
        m.blueprint.special = Dist.Modifications.specials[specialName];
      }
    }
  }
  ...
  setModuleSpecial(m, special) {
    if (m.blueprint) {
      ...
      if (m.preEngineered && m.preEngineered.blueprints) {
        this.clearModifications(m);
        const blueprintNames = Lodash.split(m.preEngineered.blueprints, ',');
  ...
  setModification(m, name, value, isAbsolute, preventUpdate, force = false) {
    ...
    if (!force && Dist.Modifications.modules[m.grp] && Dist.Modifications.modules[m.grp].modifications.indexOf(name) === -1) {
      return;
    }
        ...
        if (module != null) {
          ...
          if (blueprints && blueprints[i + 1]) {
            ...
            if (module.preEngineered && module.preEngineered.blueprints) {
              const blueprintNames = Lodash.split(module.preEngineered.blueprints, ',');
              for (const blueprintName of blueprintNames) {
            ...
            // This is a pre-engineered module with no saved blueprint, so create the blueprint structure
            module.blueprint = {};
            module.blueprint.fdname = Lodash.split(module.preEngineered.blueprints, ',')[0].trim();
            // Pre-engineered modules can be different grades
            module.blueprint.grade = module.preEngineered.grade;
            module.blueprint.grades = Dist.Modifications.blueprints[module.blueprint.fdname].grades;
            module.blueprint.name = Dist.Modifications.blueprints[module.blueprint.fdname].name;

            // If the pre-engineered module has a experimental effect, apply it
            if (module.preEngineered.experimentalEffects && module.preEngineered.experimentalEffects.length > 0) {
              ...
              const special = Lodash.find(Dist.Modifications.specials, o => o.edname === specialName);
              if (special) {
                module.blueprint.special = special;
              }
            }
            // console.log('Created blueprint structure:', module.blueprint);
            // Apply the blueprints cumulatively using the same logic as setModuleSpecial
            const blueprintNames = Lodash.split(module.preEngineered.blueprints, ',');
      ...
      if (comps && comps.hardpoints[i] !== 0) {
        ...
        if (module != null) {
          ...
          if (blueprints && blueprints[cl + i]) {
            ...
            // Apply the blueprints cumulatively
            const blueprintNames = Lodash.split(module.preEngineered.blueprints, ',');
            for (const blueprintName of blueprintNames) {
              const blueprint = getBlueprint(blueprintName.trim(), module);
              if (blueprint) {
  ...
  updateMovement() {
    ...
    this.speeds = speed(movementMass, this.speed, this.standard[1].m, this.minthrust);
    ...
    this.pitches = pitch(movementMass, this.pitch, this.standard[1].m, this.pipSpeed);
    this.topPitch = this.pitches[4];

    this.rolls = roll(movementMass, this.roll, this.standard[1].m, this.pipSpeed);
    this.topRoll = this.rolls[4];

    this.yaws = yaw(movementMass, this.yaw, this.standard[1].m, this.pipSpeed);
 ...
 */
function speed(mass, baseSpeed, thrusters, minthrust) {
 ...
 */
function calcSpeed(mass, baseSpeed, thrusters, minthrust, eng, boostFactor, boost) {
 ...
 * @param {*} INTGRPCAT
 */
function getIntCategoriesForSlot(slot) {
  ...
  for (const grp in INTGRPCAT) {
    ...
    const group = Dist.Modules.internal[grp];
    if (group) {
 ...
 */
function getHpCategoriesForSlot(slot) {
  const categories = {};
  const groupsToCheck = Object.keys(Dist.Modules.hardpoints);
  ...
  for (const grp of groupsToCheck) {
    ...
    if (HardpointCategories[grp]) {
      const moduleGroup = Dist.Modules.hardpoints[grp];
      if (moduleGroup) {
 ...
 */
function toSLEF(buildName, ship) {
  const modules = [];
